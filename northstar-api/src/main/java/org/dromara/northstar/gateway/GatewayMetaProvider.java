...
public class GatewayMetaProvider {
	...
	private Map<ChannelType, IMarketDataRepository> mdRepoMap = new EnumMap<>(ChannelType.class);
	...
	public IMarketDataRepository getMarketDataRepo(ChannelType channelType) {
	...
	public void add(ChannelType channelType, GatewaySettings settings, GatewayFactory factory, IMarketDataRepository mdRepo) {
		log.info("注册 [{}] 渠道元信息", channelType);
		if(Objects.nonNull(settings))
			settingsMap.put(channelType, settings);
		if(Objects.nonNull(factory))
			factoryMap.put(channelType, factory);
		if(Objects.nonNull(mdRepo))
			mdRepoMap.put(channelType, mdRepo);
	}
