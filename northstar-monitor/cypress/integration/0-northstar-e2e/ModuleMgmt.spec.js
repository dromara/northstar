...
describe('模组管理-测试', () => {
    ...
    describe('模组管理-基础测试', () => {
        ...
        it('应该可以启用、停用模组，模组启用时删除按钮不可用，停用时删除按钮可用', () => {
            cy.get('.el-table__row').contains('停用')
            cy.get('.el-table__row').contains('删除').should('be.enabled')
            cy.get('.el-table__row').contains('停用').parent().click()
            cy.get('.el-table__row').contains('启用')
            cy.get('.el-table__row').contains('删除').should('be.disabled')
            cy.get('.el-table__row').contains('启用').parent().click()
