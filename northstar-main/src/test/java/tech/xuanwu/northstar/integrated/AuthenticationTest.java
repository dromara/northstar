...
public class AuthenticationTest {
	...
	public void test_NS34_CorrectUserIdAndPassword() {
		ResultBean<String> result = restTemplate.postForObject("/auth/token", new NsUser("admin","123456"), ResultBean.class);
	...
	public void test_NS36_WrongUserIdAndPassword() {
		ResultBean<String> result1 = restTemplate.postForObject("/auth/token", new NsUser("KEVIN","123456"), ResultBean.class);
		assertThat(result1.getStatus()).isEqualTo(ReturnCode.AUTH_ERR);
		assertThat(StringUtils.hasText(result1.getMessage())).isTrue();
		
		ResultBean<String> result2 = restTemplate.postForObject("/auth/token", new NsUser("admin","00000"), ResultBean.class);
