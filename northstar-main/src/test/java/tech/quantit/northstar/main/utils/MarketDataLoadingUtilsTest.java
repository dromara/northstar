...
class MarketDataLoadingUtilsTest {
	...
	void testGetCurrentTradeDay() {
		...
		assertThat(utils.getCurrentTradeDay(sometime.toInstant(ZoneOffset.ofHours(8)).toEpochMilli())).isEqualTo(LocalDate.of(2022, 6, 1));
		
		// 日盘情况
		LocalDateTime sometime2 = LocalDateTime.of(LocalDate.of(2022, 5, 31), LocalTime.of(15, 0));
		assertThat(utils.getCurrentTradeDay(sometime2.toInstant(ZoneOffset.ofHours(8)).toEpochMilli())).isEqualTo(LocalDate.of(2022, 5, 31));
		
		// 周五晚夜盘情况
		LocalDateTime sometime3 = LocalDateTime.of(LocalDate.of(2022, 5, 27), LocalTime.of(22, 0));
		assertThat(utils.getCurrentTradeDay(sometime3.toInstant(ZoneOffset.ofHours(8)).toEpochMilli())).isEqualTo(LocalDate.of(2022, 5, 30));
	}
