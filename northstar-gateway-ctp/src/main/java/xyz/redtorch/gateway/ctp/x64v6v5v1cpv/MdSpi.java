...
import xyz.redtorch.gateway.ctp.x64v6v5v1cpv.api.CThostFtdcMdApi;
import xyz.redtorch.gateway.ctp.x64v6v5v1cpv.api.CThostFtdcMdSpi;
...
import xyz.redtorch.gateway.ctp.x64v6v5v1cpv.api.CThostFtdcSpecificInstrumentField;
...
public class MdSpi extends CThostFtdcMdSpi {
	...
	public void connect() {
		...
		try {
			String mdHost = GatewayConstants.SMART_CONNECTOR.bestEndpoint(settings.getBrokerId());
			String mdPort = settings.getMdPort();
	...
	private void login() {
		if (StringUtils.isEmpty(settings.getBrokerId()) 
				|| StringUtils.isEmpty(settings.getUserId()) 
		...
		try {
			...
			userLoginField.setBrokerID(settings.getBrokerId());
	...
	public void OnRtnDepthMarketData(CThostFtdcDepthMarketDataField pDepthMarketData) {
		if (pDepthMarketData != null) {
			try {
				...
				tickBuilder.setGatewayId(ChannelType.CTP_SIM.toString());
				tickBuilder.setChannelType(ChannelType.CTP_SIM.toString());
