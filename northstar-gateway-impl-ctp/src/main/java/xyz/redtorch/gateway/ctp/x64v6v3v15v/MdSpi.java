	...
	public void OnRtnDepthMarketData(CThostFtdcDepthMarketDataField pDepthMarketData) {
		if (pDepthMarketData != null) {
			try {
				...
				actionDay = StringUtils.isEmpty(actionDay) ? LocalDate.now().format(CommonConstant.D_FORMAT_INT_FORMATTER) : actionDay;

				// 接口返回的updateTime数据格式为 HH:mm:ss
				String updateTimeStr = pDepthMarketData.getUpdateTime().replaceAll(":", "");
				updateTimeStr = StringUtils.isEmpty(updateTimeStr) ? LocalTime.now().format(CommonConstant.T_FORMAT_INT_FORMATTER) : updateTimeStr;
				...
				ctpGatewayImpl.getEventEngine().emitEvent(EventType.TICK, ctpGatewayImpl.getGateway().getName(), tick);
			} catch (Throwable t) {
